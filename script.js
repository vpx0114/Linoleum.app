...
const localStore = {
  async get(key){
    try{
      if(typeof db !== 'undefined' && db){
        const doc = await db.collection('linoleum_data').doc(key).get();
        if(doc.exists){
    ...
    }catch(e){ console.log("Bulutdan o'qishda xatolik, mahalliy xotiradan olinadi:", e.message); }
    try{
  ...
  async set(key, value){
    ...
    try{
      if(typeof db !== 'undefined' && db){
        await db.collection('linoleum_data').doc(key).set({ value, updatedAt: new Date().toISOString() });
      }
      return { key, value };
    }catch(e){
      console.log("Bulutga yozishda xatolik (mahalliy saqlandi):", e.message);
      return { key, value };
    }
  }
};
...
async function loadData(){
  ...
  try{
    const mv = await localStore.get('harakatlar');
...
async function saveMovements(){
  try{
    const r = await localStore.set('harakatlar', JSON.stringify(movements));
    if(!r) showToast('Saqlashda xatolik');
  }catch(e){ showToast('Saqlashda xatolik: ' + e.message); }
}
...
function renderKassaList(){
  ...
  products.forEach(p=>{
    ...
    const thumb = p.image
      ...
      : `<div class="pthumb-placeholder">${ICONS.image}</div>`;
    row.innerHTML = `
      <div class="pleft">
        ${thumb}
        <div>
...
async function handleKeldiImageFile(input){
  const file = input.files && input.files[0];
  if(!file) return;
  try{
    keldiImageData = await resizeImageFile(file, 300);
    document.getElementById('keldiImagePreviewWrap').innerHTML = `<img class="img-preview" src="${keldiImageData}">`;
  }catch(e){
    showToast('Rasmni yuklashda xatolik');
  }
}
...
async function addProduct(){
  ...
  const newProd = {id: 'prod_' + Date.now(), name, doc, price, qty, width: isNaN(width) ? null : width, image: newImageData || null};
  ...
  document.getElementById('newWidth').value = '1.5';
...
async function saveEditProduct(id){
  ...
  const widthEl = document.getElementById('edit-width');
  ...
  product.price = price;
  product.qty = qty;
  if(!isNaN(width)) product.width = width;
...
function renderOmborTable(){
  ...
  filteredProducts.sort((a, b) => {
    const wa = (a.width === null || a.width === undefined || isNaN(a.width)) ? Infinity : a.width;
    const wb = (b.width === null || b.width === undefined || isNaN(b.width)) ? Infinity : b.width;
    return wa - wb;
  });
  ...
  filteredProducts.forEach(p=>{
    ...
    if(isEditing){
      ...
      const editWidthOptions = [1.5,2,2.5,3,3.5,4].map(w =>
        `<option value="${w}" ${Number(p.width) === w ? 'selected' : ''}>${w}</option>`
      ).join('');
      ...
      tr.innerHTML = `
        <td class="edit-cell" colspan="7">
          ...
          <div class="field"><label>Eni</label><select id="edit-width">${editWidthOptions}</select></div>
          <div class="edit-actions">
            <button class="secondary" onclick="saveEditProduct('${p.id}')">${ICONS.check} Saqlash</button>
            <button class="secondary" onclick="cancelEditProduct()">${ICONS.xCircle} Bekor qilish</button>
          </div>
        </td>
      `;
      body.appendChild(tr);
    ...
    const thumb = p.image
      ...
      : `<div class="table-thumb-placeholder">${ICONS.image}</div>`;

    tr.innerHTML = `
      <td>${thumb}</td>
      <td>${escapeHtml(p.name)}</td>
      <td>${escapeHtml(p.doc || '—')}</td>
      <td class="num">${(p.width === null || p.width === undefined || p.width === '') ? '—' : p.width}</td>
      <td class="num">${fmt(p.price)}</td>
      <td class="num ${p.qty < 3 ? 'low' : ''}">${p.qty}</td>
      <td>
        ${isPending
          ? `<button class="icon-btn del wide" title="Ha, o'chirish" onclick="confirmDeleteProduct('${p.id}')">${ICONS.check} Ha, o'chir</button>
             <button class="icon-btn wide" title="Bekor qilish" onclick="cancelDeleteProduct()">${ICONS.xCircle} Bekor</button>`
          : `<button class="icon-btn add" title="Qoldiq qo'shish (+1 metr)" onclick="addStockQty('${p.id}')">${ICONS.plusCircle}</button>
             <button class="icon-btn" title="Tahrirlash" onclick="startEditProduct('${p.id}')">${ICONS.pencil}</button>
             <button class="icon-btn del" title="O'chirish" onclick="requestDeleteProduct('${p.id}')">${ICONS.trash}</button>`
        }
      </td>
    `;
    body.appendChild(tr);
...
// ---------- OMBOR: YANGI / KELDI / KETDI REJIMLARI ----------
let omborMode = 'yangi';

function setOmborMode(mode){
  omborMode = mode;
  ['yangi','keldi','ketdi'].forEach(m=>{
    const box = document.getElementById('omborMode' + m.charAt(0).toUpperCase() + m.slice(1));
    const btn = document.getElementById('omborBtn' + m.charAt(0).toUpperCase() + m.slice(1));
...
// ---------- KELDI (OMBORGA MAHSULOT KELISHI) ----------
async function addKeldi(){
  const name = document.getElementById('keldiName').value.trim();
  const doc = document.getElementById('keldiDoc').value.trim();
  const price = parseFloat(document.getElementById('keldiPrice').value);
  const qty = parseFloat(document.getElementById('keldiQty').value);
  const width = document.getElementById('keldiWidth').value;

  if(!name){ showToast('Nomini kiriting'); return; }
  if(isNaN(price) || price < 0){ showToast("Narxni to'g'ri kiriting"); return; }
  if(isNaN(qty) || qty <= 0){ showToast("Miqdorni to'g'ri kiriting"); return; }

  const widthNum = parseFloat(width);

  let product = doc ? products.find(p => (p.doc || '').trim().toLowerCase() === doc.toLowerCase()) : null;

  if(product){
    product.qty = Math.round((product.qty + qty) * 100) / 100;
    product.price = price;
    if(!isNaN(widthNum)) product.width = widthNum;
    if(name) product.name = name;
    if(keldiImageData) product.image = keldiImageData;
  } else {
    product = {id: 'prod_' + Date.now(), name, doc, price, qty, width: isNaN(widthNum) ? null : widthNum, image: keldiImageData || null};
  ...
  movements.push({
    ...
    qty,
    width,
    image: keldiImageData || null
  });

  await Promise.all([saveProducts(), saveMovements()]);

  document.getElementById('keldiName').value = '';
  document.getElementById('keldiDoc').value = '';
  document.getElementById('keldiPrice').value = '';
  document.getElementById('keldiQty').value = '';
  keldiImageData = null;
  document.getElementById('keldiImageInput').value = '';
  document.getElementById('keldiImagePreviewWrap').innerHTML = '<div class="img-preview-placeholder">Rasm yo\'q</div>';
  showToast(`Keldi: ${qty} m qo'shildi (${product.name})`);
  renderAll();
}

// ---------- KETDI (OMBORDAN MAHSULOT CHIQISHI) ----------
function renderKetdiProductOptions(){
  const sel = document.getElementById('ketdiProductSelect');
  if(!sel) return;
  ...
  products.forEach(p=>{
    ...
    opt.value = p.id;
    opt.textContent = `${p.name}${p.doc ? ' (' + p.doc + ')' : ''} — ${p.qty} m qoldi`;
    sel.appendChild(opt);
  });
  if(products.some(p => p.id === prevValue)) sel.value = prevValue;
}

function updateKetdiSum(){
  const qty = parseFloat(document.getElementById('ketdiQty')?.value) || 0;
  const price = parseFloat(document.getElementById('ketdiPrice')?.value) || 0;
  const sumEl = document.getElementById('ketdiSum');
  if(sumEl) sumEl.value = fmt(qty * price);
}

async function addKetdi(){
  const productId = document.getElementById('ketdiProductSelect').value;
  const width = document.getElementById('ketdiWidth').value;
  const qty = parseFloat(document.getElementById('ketdiQty').value);
  const price = parseFloat(document.getElementById('ketdiPrice').value);
  const destination = document.getElementById('ketdiDestination').value.trim();

  const product = products.find(p => p.id === productId);
  if(!product){ showToast('Mahsulotni tanlang'); return; }
  if(isNaN(qty) || qty <= 0){ showToast("Metrni to'g'ri kiriting"); return; }
  if(isNaN(price) || price < 0){ showToast("Narxni to'g'ri kiriting"); return; }
  if(qty > product.qty){ showToast('Omborda yetarli emas: ' + product.qty + ' m bor'); return; }
  ...
  document.getElementById('ketdiQty').value = '';
  document.getElementById('ketdiPrice').value = '';
  document.getElementById('ketdiDestination').value = '';
  updateKetdiSum();
  showToast(`Ketdi: ${qty} m chiqarildi (${fmt(total)} so'm)`);
  renderAll();
}

// ---------- KELDI/KETDI TARIXI ----------
function renderMovementsHistory(){
  const el = document.getElementById('movementsHistoryList');
  const emptyNote = document.getElementById('movementsEmpty');
  if(!el) return;
  el.innerHTML = '';
  if(emptyNote) emptyNote.style.display = movements.length === 0 ? 'block' : 'none';

  const sorted = [...movements].sort((a,b)=> new Date(b.timestamp) - new Date(a.timestamp)).slice(0, 30);
  sorted.forEach(m=>{
    const row = document.createElement('div');
    row.className = 'movement-row';
    const dateStr = formatDateTime(m.timestamp);
    if(m.type === 'keldi'){
      const thumb = m.image ? `<img class="table-thumb" src="${m.image}">` : '';
      row.innerHTML = `
        <div class="movement-left">
          <span class="movement-badge keldi">Keldi</span>
          ${thumb}
          <div class="movement-info">
            <div class="movement-name">${escapeHtml(m.name)}</div>
            <div class="movement-meta">${escapeHtml(m.doc || '—')} · eni ${m.width || '—'} · ${dateStr}</div>
          </div>
        </div>
        <div class="movement-val keldi">+${m.qty} m</div>
      `;
    } else {
      row.innerHTML = `
        <div class="movement-left">
          <span class="movement-badge ketdi">Ketdi</span>
          <div class="movement-info">
            <div class="movement-name">${escapeHtml(m.name)}</div>
            <div class="movement-meta">${escapeHtml(m.doc || '—')} · eni ${m.width} · ${escapeHtml(m.destination || '—')} · ${dateStr}</div>
          </div>
        </div>
        <div class="movement-val ketdi">−${m.qty} m</div>
      `;
    }
    el.appendChild(row);
  });
...
function renderScanResults(matches, context){
  ...
  valid.forEach(m=>{
    ...
    const widthStr = (product.width === null || product.width === undefined || product.width === '') ? '—' : product.width;
    let actionBtnHtml = '';
    ...
    row.innerHTML = `
      ...
      <div class="scan-result-info">
        ...
        <div style="font-size: 12px; color: #666; margin-top: 2px;">Aniqlik: ${score}% | Eni: ${widthStr} | Qoldiq: ${stockQty} m</div>
...
function exportBackup(){
  const data = { products, sales, expenses, movements, exportedAt: new Date().toISOString() };
...
function handleRestoreFile(input){
  ...
  reader.onload = () => {
    ...
    try{
      ...
      statusEl.innerHTML = `Faylda: ${(data.products||[]).length} mahsulot, ${(data.sales||[]).length} sotuv, ${(data.expenses||[]).length} chiqim, ${(data.movements||[]).length} Keldi/Ketdi topildi. Joriy ma'lumotlar shular bilan almashtiriladi.
...
async function applyRestore(){
  ...
  movements = pendingRestoreData.movements || [];
  await Promise.all([saveProducts(), saveSales(), saveExpenses(), saveMovements()]);
...
function renderExpenseList(){
  ...
  sorted.forEach(e=>{
    ...
    row.innerHTML = `
      ...
      ${isPending
        ? `<button class="icon-btn del wide" title="Ha, o'chirish" onclick="confirmDeleteExpense('${e.id}')">${ICONS.check} Ha</button>
           <button class="icon-btn wide" title="Bekor qilish" onclick="cancelDeleteExpense()">${ICONS.xCircle} Bekor</button>`
        : `<button class="icon-btn del" title="O'chirish" onclick="requestDeleteExpense('${e.id}')">${ICONS.trash}</button>`
      }
    `;
    el.appendChild(row);
  });
...
async function sendTelegramReport(){
  ...
  const todaySales = sales.filter(s => s.timestamp && s.timestamp.slice(0, 10) === todayStr);
  const todayKetdi = movements.filter(m => m.type === 'ketdi' && m.timestamp && m.timestamp.slice(0, 10) === todayStr);
  const todayKeldi = movements.filter(m => m.type === 'keldi' && m.timestamp && m.timestamp.slice(0, 10) === todayStr);

  if(todaySales.length === 0 && todayKetdi.length === 0 && todayKeldi.length === 0){
    showToast("Bugun sotuv yoki Keldi/Ketdi bo'lmagan");
    return;
  }
  ...
  // Ketdi (ombordan chiqarilgan tovarlar) bo'limi
  let ketdiText = "";
  let ketdiTotalSum = 0;
  if(todayKetdi.length){
    ketdiText += `--\nKetdi:\n`;
    todayKetdi.forEach((m, i) => {
      ketdiTotalSum += (m.total != null ? m.total : (m.qty * m.price));
      ketdiText += `${i+1}. Hujjat raqami: ${m.doc || '—'}\nQayerga: ${m.destination || '—'}\n eni: ${m.width} | metr: ${m.qty}\n`;
    });
    ketdiText += `\n`;
  }

  // Keldi (omborga kelib tushgan tovarlar) bo'limi
  let keldiText = "";
  if(todayKeldi.length){
    keldiText += `-----\nKeldi:\n`;
    todayKeldi.forEach((m, i) => {
      keldiText += `${i+1}. Hujjat raqami: ${m.doc || '—'}\n eni: ${m.width || '—'} | metr: ${m.qty}\n`;
    });
    keldiText += `\n`;
  }
  ...
  const umumiyKassa = totalSalesSum - todayExpenses;
  ...
  msg += `-----\n`;
  if(todayKetdi.length){
    msg += `Ketdi (umumiy summa): ${fmt(ketdiTotalSum)} so'm\n`;
  }
...
document.getElementById('addKeldiBtn').addEventListener('click', addKeldi);
document.getElementById('addKetdiBtn').addEventListener('click', addKetdi);
