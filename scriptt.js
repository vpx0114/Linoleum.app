...
async function addKeldi(){
  ...
  if(product){
    ...
    product = {id: 'prod_' + Date.now(), name, doc, price, qty, image: keldiImageData || null};
...
async function sendTelegramReport(){
  ...
  const todayExpenseList = expenses
    .filter(e => e.timestamp && e.timestamp.slice(0, 10) === todayStr);
  const todayExpenses = todayExpenseList.reduce((sum, e) => sum + (e.amount || 0), 0);
  ...
  msg += keldiText;
  if(todayKetdi.length){
    msg += `__\nKetdi ummumiy summa: ${fmt(ketdiTotalSum)} so'm\n`;
  }
  msg += `----\n`;
  if(todayExpenseList.length){
    todayExpenseList.forEach(e => {
      msg += `Chiqim: ${fmt(e.amount)} so'm\nizoh: ${e.note || '—'}\n`;
    });
  } else {
    msg += `Chiqim: 0 so'm\n`;
  }
